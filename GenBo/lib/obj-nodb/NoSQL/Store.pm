...
sub _build_storage {
    ...
    return NoSQL::Storage->new( store => $self );
}
...
sub _build_type_map { 
    my $self = shift;
    require NoSQL::TypeMap;
    return NoSQL::TypeMap->new();
}
...
sub model {
    ...
    if ( @_ > 1 ) {
        ...
        return $self->_model->{ $name } ||= NoSQL::Model->new( store => $self, name => $name );
    }
}
...
sub _build_stash {
    require NoSQL::Stash;
    my $self = shift;
    my $stash = NoSQL::Stash->new( store => $self );
    return $stash;
}

require NoSQL::ClassScaffold;

has schema_class_scaffold => qw/ is ro lazy_build 1 /;
sub _build_schema_class_scaffold { return NoSQL::ClassScaffold->new->become_Schema }
has schema_class => qw/ is ro lazy_build 1 /;
sub _build_schema_class {
    my $self = shift;
    my $class = $self->schema_class_scaffold->package;

    my $store_result_class_scaffold = NoSQL::ClassScaffold->new->become_ResultClass_Store;
...
sub _connect {
    ...
    if ( ! $self->storage->table_exists( '__Store__' ) ) {
     ...
     system("chmod a+w $database_file") unless $exists;
    return $schema;
}
