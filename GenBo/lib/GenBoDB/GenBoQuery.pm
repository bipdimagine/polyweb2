...
GenBoQuery : Use to interrogate the table GENBO of the database
...
=head1 DESCRIPTION
+
GenBoQuery provides a set of functions to interrogate the table GENBO of the database
...
my $selectGenbo = "select GENBO_ID as id, name as name, type_genbo_id as type_id , origin_id as origin_id";

=head2 getGenboById
	Title   : getGenboById
 	Usage   : GenBoQuery::getGenboById($dbh,$GenBoId);
...
=head2 getGenboByName
	Title   : getGenboByName
 	Usage   : GenBoQuery::getGenboByName($dbh,$GenBoName,$typeId,$projectId);
...
=head2 getOrigin
	Title   : getOrigin
 	Usage   : GenBoQuery::getOrigin($dbh,$projectName);
 	Function: Get the origin record corresponding to the specified project name
 	Returns : A hash table of the record
 	Args    : A string corresponding to the name of the searched origin
	Note    : 
=cut
...
sub getOrigin {
	my ($dbh,$name) =@_;
	my $query = qq{
		select project_id as id, type_project_id as type from Polyproject.projects  where name='$name';
	};
	my $sth = $dbh->prepare($query);
	$sth->execute();
	my $s = $sth->fetchrow_hashref();	
...
=head2 getRelatedDownObjects
	Title   : getRelatedDownObjects
 	Usage   : GenBoQuery::getRelatedDownObjects($dbh,$GenBoId,$typeRelationId);
...
=head2 getSequenceId
	Title   : getSequenceId
 	Usage   : GenBoQuery::getSequenceId($dbh,$sequence);
...
=head2 getDescription
	Title   : getDescription
 	Usage   : GenBoQuery::getDescription($dbh,$genBoId, $typeId);
...
=head2 getSequence
	Title   : getSequence
 	Usage   : GenBoQuery::getSequence($dbh,$genBoId);
...
=head2 existsAnnexTypeOnGenBo
	Title   : existsAnnexTypeOnGenBo
 	Usage   : GenBoQuery::existsAnnexTypeOnGenBo($dbh,$genBoId, $methodId);
...
=head2 isValid
	Title   : isValid
 	Usage   : GenBoQuery::isValid($dbh,$genBoId);
...
=head2 getAllMethods
	Title   : getAllMethods
 	Usage   : GenBoQuery::getAllMethods($dbh,$projectId);
...
=head2 getVariationOnContigByPosAndSeq
	Title   : getVariationOnContigByPosAndSeq
 	Usage   : GenBoQuery::getVariationOnContigByPosAndSeq($dbh,$contig,$position,$sequence);
...
=head2 existsObjectOnContigByVariationHash
	Title   : existsObjectOnContigByVariationHash
 	Usage   : GenBoQuery::existsObjectOnContigByVariationHash($dbh,$hash_var);
 	Function: Get the GenBo id of the variation on the specified contig at the sepcified position and with the sepcified sequence 
 	Returns : A integer corresponding to the searched GenBo id
 	Args    : A GenBoContig
 		An integer corresponding to the searched position
...
sub existsObjectOnContigByVariationHash {
	...
	my $sql = qq{SELECT g.GENBO_ID FROM GENBO g,RELATION r, SEQUENCE s, Polyproject.type_genbo t where t.name = "$type"  and t.type_genbo_id=g.type_genbo_id and r.genbo_id='$contig_id' and r.genbo2_id=g.genbo_id and r.start1='$start' and r.end1='$end' and g.sequence_id=s.sequence_id and s.sequence='$seq';};
	return  connect::returnOneVal($dbh,$sql); 
...
sub existsObjectsByPosition {
	...
	my $sql = qq{ select GENBO_ID as id  FROM DEJAVU_STATIC D,  Polyproject.type_genbo t where
...
=head2 isBipd
	Title   : isBipd
 	Usage   : GenBoQuery::isBipd($dbh,$genBoId);
