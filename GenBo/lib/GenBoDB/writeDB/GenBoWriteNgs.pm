...
=head2 insertSequence
	Title   : insertSequence
 	Usage   : GenBoWriteNgs::insertSequence($dbh,$GenBoId,$sequence);
...
sub insertSequence {
	...
	my $s = GenBoQueryNgs::getSequenceId($dbh,$sequence);
	...
	unless (exists $s->{id}) {
		$dbh->do($query);
		$s =  GenBoQueryNgs::getSequenceId($dbh,$sequence);
	}
	my $sequenceId=$s->{id};
	my $sql = qq {
...
=head2 createGenBo
	Title   : createGenBo
 	Usage   : GenBoWriteNgs::createGenBo($dbh,$name,$typeId,$projectId);
...
sub createGenBo{
	...
	my $r =  GenBoQueryNgs::getGenboByName($dbh,$name,$type->id,$origin);
...
=head2 createOrigin
	Title   : createOrigin
 	Usage   : GenBoWriteNgs::createOrigin($dbh,$name,$projectType);
...
sub createOrigin{
	my ($dbh,$name,$type) = @_;
	my $typeId = GenBoQueryNgs::getOriginType($dbh, $type);
	my $r =  GenBoQueryNgs::getOrigin($dbh,$name);
	my $query = qq{
		insert into ORIGIN (NAME,TYPE_ORIGIN_ID) values ('$name',$typeId->{id});
	};
	unless (  defined $r) {
		$dbh->do($query) ;
		$r =  GenBoQueryNgs::getOrigin($dbh,$name);
	}
	return $r->{id};
}
...
=head2 createGenBoWithCompteur
	Title   : createGenBoWithCompteur
 	Usage   : GenBoWriteNgs::createGenBoWithCompteur($dbh,$compteur, $typeId, $projectId);
...
sub createGenBoWithCompteur{
	...
	#warn $full_name." ".$cpt;
	my $r =  GenBoQueryNgs::getGenboByName($dbh,$full_name,$type->id(),$origin);
...
=head2 createGenBoVariation
	Title   : createGenBoVariation
 	Usage   : GenBoWriteNgs::createGenBoVariation($dbh,$name, $typeId, $projectId);
 	Function: Create a record in the table GENBO for the variation of the specified name,type and project id
  	Returns : The id of the inserted GenBo (int)
 	Args    : A connection to the database, the name of the variation (string), the type of the GenBo (int), the project id of the variation (int)
	Note    : Check if the variation name doesn't already exists
...
sub createGenBoVariation{
	...
	my $r =  GenBoQueryNgs::getGenboByName($dbh,$name,$type,$origin);
	if (defined $r){
	...
	unless (  defined $r) {
		$dbh->do($query) ;
		$r =  GenBoQueryNgs::getGenboByName($dbh,$name,$type,$origin);
	}
	return $r->{id};
}
...
=head2 createGenBoIndel
	Title   : createGenBoIndel
 	Usage   : GenBoWriteNgs::createGenBoIndel($dbh,$name, $typeId, $projectId);
...
=head2 insertDescription
	Title   : insertDescription
 	Usage   : GenBoWriteNgs::insertDescription($dbh,$GenBoId, $typeId, $name);
...
sub insertDescription{
	...
	my $des=GenBoQueryNgs::getDescription($dbh,$genboId,$type);
	my $query = qq{
	...
	unless (  defined $des) {
		$dbh->do($query) ;
		$des = GenBoQueryNgs::getDescription($dbh,$genboId,$type);
	}
	return $des->{id};
}
...
=head2 valider
	Title   : valider
 	Usage   : GenBoWriteNgs::valider($dbh,$GenBoId,$status);
