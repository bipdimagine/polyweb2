...
GenBoRelationQuery : Use to interrogate the table RELATION of the database
...
GenBoRelationQuery provides a set of functions to interrogate the table RELATION of the database

=head1 METHODS

=cut

package GenBoRelationQuery;
use Carp;
use strict;
...
=head2 getRelationType
	Title   : getRelationType
 	Usage   : GenBoRelationQuery::getRelationType($dbh,$name);
...
sub getRelationType {
	my ($dbh,$name) = @_;
	my $query = qq{
		select type_relation_id as id from Polyproject.type_relation tr 
		where tr.name= '$name';
	};
...
sub getAllRelationType {
	my ($dbh,$name) = @_;
	my $query = qq{
		select type_relation_id as id ,name as name from Polyproject.type_relation tr ;		
	};
...
=head2 getRelationsDown
	Title   : getRelationsDown
 	Usage   : GenBoRelationQuery::getRelationsDown($dbh,$GenBoId, $relationTypeId);
...
sub getRelationsDown {
	...
	unless (exists $query{down}){
		...
		select  relation_id as rid, GENBO2_ID as '-id', go.name as '-name', start2 as start,end2 as end, strand as strand,start1 as start2, end1 as end2, method as method
		  from RELATION r , ORIGIN_GENBO g, GENBO go ,  Polyproject.project_release pr
...
=head2 getRelationsUp
	Title   : getRelationsUp
 	Usage   : GenBoRelationQuery::getRelationsUp($dbh,$GenBoId, $relationTypeId);
...
=head2 getRelationId
	Title   : getRelationId
 	Usage   : GenBoRelationQuery::getRelationId($dbh,$GenBoId1, $GenBoId2, $relationTypeId);
...
=head2 getMethodId
	Title   : getMethodId
 	Usage   : GenBoRelationQuery::getMethodId($dbh,$name);
...
sub getMethodId {
	my ($dbh,$name) = @_ ;
	my $sql = qq{select method_id as ID from Polyproject.methods where name = '$name'};
...
=head2 getAnnexByRelationId
	Title   : getAnnexByRelationId
 	Usage   : GenBoRelationQuery::getAnnexByRelationId($dbh,$relationId);
...
sub getAnnexByRelationId {
	my ($dbh,$id) = @_ ;
	my $sql = qq{select annex_id as id, score as score,text as text,name as method_name,score2 as score2,score3 as score3 ,score4 as score4,ho as homozygote,he as heterozygote  FROM RELATION_ANNEX r , Polyproject.methods m  where m.method_id=r.method_id and relation_id=$id};
	my $sth = $dbh->prepare($sql);
...
=head2 isValid
	Title   : isValid
 	Usage   : GenBoRelationQuery::isValid($dbh,$relationId);
...
=head2 getFirstAndLastVariationPosition
	Title   : getFirstAndLastVariationPosition
 	Usage   : GenBoRelationQuery::getFirstAndLastVariationPosition($dbh,$Id);
