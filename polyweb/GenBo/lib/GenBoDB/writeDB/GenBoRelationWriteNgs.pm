...
=head2 _addRelation
	Title   : _addRelation
 	Usage   : GenBoRelationWriteNgs::_addRelation($dbh,$GenBoId1,$GenBoId2,$typeRelationId,$start,$end,$start2,$end2,$strand,$allele,$consequence,$methodId);
...
sub _addRelation {
	...
	my $r = GenBoRelationQueryNgs::getRelationId($dbh,$genboId1,$genboId2,$type);
	GenBoDelete::deleteByRelationId($dbh,$r->{id})if defined $r ;
#	return $r->{id} if defined $r;
	my $query = qq{
		insert into Polyexome_HG19.RELATION (GENBO_ID,GENBO2_ID,TYPE_RELATION_ID,START1,END1,START2,END2,STRAND,SCORE,TEXT,CONSEQUENCE,METHOD) 
		values ('$genboId1','$genboId2','$type','$start','$end','$start2','$end2','$strand','$score','$text','$consequence','$method');
	};
	$dbh->do($query) ;
	
	$r = GenBoRelationQueryNgs::getRelationId($dbh,$genboId1,$genboId2,$type);
	return $r->{id};
...
=head2 addRelation
	Title   : addRelation
 	Usage   : GenBoRelationWriteNgs::addRelation($dbh,$GenBoId1,$GenBoId2,$typeId,$start,$end,$start2,$end2,$strand);
 	Function: Insert a relation in the table RELATION for the specified GenBos ids with the differents attributes passed to the method
 	Returns : The id of the inserted relation (int)
...
=head2 addAnnex
	Title   : addAnnex
 	Usage   : GenBoRelationWriteNgs::addAnnex($dbh,$relationId,$methodId,$score,$allele);
...
sub addAnnex {
	...
	my $query = qq{ insert into Polyexome_HG19.RELATION_ANNEX(relation_id,method_id,score,text) values($relationid,$method_id,$score,"$text") 
...
sub newAnnexForRelation {
	...
	my $he = $args->{he};
	my $ho = $args->{ho};
	my $query = qq{ insert into Polyexome_HG19.RELATION_ANNEX(relation_id,method_id,score,text,score2,score3,score4,he,ho) values($relation_id,$method_id,$score,"$text",$score_2,$score_3,$score_4,$he,$ho) 
...
=head2 valider
	Title   : valider
 	Usage   : GenBoRelationWriteNgs::valider($dbh,$relationId,$status);
