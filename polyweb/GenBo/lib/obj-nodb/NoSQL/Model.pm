...
sub field {
    ...
    my $field = $self->field_map->{ $name } = NoSQL::Model::Field->new( name => $name );
    $field->setup( $self, @_ );
    return $field;
}
...
sub _build_index {
    require NoSQL::Model::Index;
    my $self = shift;
    return unless $self->searchable;
    return NoSQL::Model::Index->new( model => $self );
}
...
sub get_ids {
    ...
    my $t = $self->store->dbh->selectall_hashref($statement,'key');
